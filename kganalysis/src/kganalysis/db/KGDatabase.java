...
public class KGDatabase {
    ...
    // indicator whether the database is started
	...
	registerProcedures(graphDb, 
		PageRankWriteProc.class, BetweennessCentralityWriteProc.class, DegreeCentralityWriteProc.class, 
		LouvainWriteProc.class, LabelPropagationWriteProc.class, WccWriteProc.class,
		GraphRefactoring.class, Create.class, Strings.class, Nodes.class);
	isStarted = true;
    }
    ...
    // Shut down the database manually
	...
	try (Transaction tx = graphDb.beginTx()) {
	    tx.execute("MATCH (n:elements)\n" + " CALL apoc.create.addLabels(n, [ n.layer ])\n" + " YIELD node\n"
		    + " RETURN node\n");
	    tx.commit();
	}
    }

    public void addRelationshipLabels() {
	try (Transaction tx = graphDb.beginTx()) {
	    tx.execute("MATCH (f)-[rel]->(b)\n" + " CALL apoc.refactor.setType(rel, rel.type)\n"
		    + " YIELD input, output\n" + " RETURN input, output\n");
